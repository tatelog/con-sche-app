...
デュアルライセンスです。どちらかを選んでご利用ください。

### 1. GNU AGPL-3.0-or-later（無償）

[GNU Affero General Public License v3.0](LICENSE) の条件で自由に利用・改変・再配布できます。

- 利用・改変・社内利用・業務利用: **無償**
- ネットワーク越しに第三者へ提供する場合、改変部分を含む全ソースコードの公開義務が生じます（AGPL 第13条）
- 派生物も AGPL-3.0 で配布する必要があります

### 2. 商用ライセンス（別途契約）

AGPL の公開義務を負わずに利用したい場合は、建ログ株式会社との個別契約になります。
次のようなケースが該当します。

- ソースを公開せずに自社製品・サービスへ組み込みたい（OEM）
- ソースを公開せずに第三者向けのマネージドサービスとして提供したい
- 独自のAPI基盤・自動処理基盤として提供したい
